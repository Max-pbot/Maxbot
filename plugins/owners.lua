...
local function lock_group_floodmod(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return 'User ['..user_id..' ]floode gorooh ghofl ast!'
  else
    data[tostring(target)]['settings']['flood'] = 'yes'
    save_data(_config.moderation.data, data)
  return 'User ['..user_id..' ]floode gorooh ghofl shod!'
  end
end
...
local function unlock_group_floodmod(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return 'User ['..user_id..' ]floode gorooh baz ast!'
  else
    data[tostring(target)]['settings']['flood'] = 'no'
    save_data(_config.moderation.data, data)
  return 'User ['..user_id..' ]floode gorooh baz shod!'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'User ['..user_id..' ]addmembere gorooh ghofl ast!'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'User ['..user_id..' ]addmembere gorooh ghofl shod!'
end

local function unlock_group_membermod(msg, data, target)
  local group_member_lock = data[tostring(target)]['settings']['lock_member']
  if group_member_lock == 'no' then
    return 'User ['..user_id..' ]addmembere gorooh baz ast!'
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
  return 'User ['..user_id..' ]addmembere gorooh baz shod!'
  end
end
...
local function unlock_group_photomod(msg, data, target)
  ...
  if group_photo_lock == 'no' then
      return 'User ['..user_id..' ]axe gorooh baz ast!'
  else
      data[tostring(target)]['settings']['lock_photo'] = 'no'
      save_data(_config.moderation.data, data)
  return 'User ['..user_id..' ]axe gorooh baz shod!'
  end
end
...
local function set_rules(target, rules)
  ...
  save_data(_config.moderation.data, data)
  return 'User ['..user_id..' ]taghire ghavanine gorooh be:\n'..rules
end
...
local function set_description(target, about)
  ...
  save_data(_config.moderation.data, data)
  return 'User ['..user_id..' ]taghire etelaate gorooh be:\n'..about
end
  ...
  if msg.to.type ~= 'chat' then
    ...
    if matches[2] == 'ban' then
      ...
      if not is_owner2(msg.from.id, chat_id) then
        return "User ["..user_id.." ]shoma modire in gorooh nistid!"
      end
      ...
      savelog(matches[1], name.." ["..msg.from.id.."]User ["..user_id.." ] User ban shode ast! ".. matches[3])
      return 'User '..user_id..' ban shode ast'
    end
    ...
    if tonumber(matches[3]) == tonumber(our_id) then return false end
      ...
      local user_id = matches[3]
      if tonumber(matches[3]) == tonumber(msg.from.id) then 
        return "User ["..user_id.." ]shoma nmitoonid khodetono unban konid!"
      end
      local hash =  'banned:'..matches[1]
      redis:srem(hash, user_id)
      local name = user_print_name(msg.from)
      savelog(matches[1], name.." ["..msg.from.id.."]User ["..user_id.." ] User unban shode ast! ".. matches[3])
    ...
    if matches[2] == 'kick' then
      ...
      local user_id = matches[3]
      if tonumber(matches[3]) == tonumber(msg.from.id) then 
        return "User ["..user_id.." ]shoma nmitoonid khodetono kick konid!"
      end
      kick_user(matches[3], matches[1])
      local name = user_print_name(msg.from)
      savelog(matches[1], name.." ["..msg.from.id.."]User ["..user_id.." ] User kick shod! ".. matches[3])
      return 'User '..user_id..' kick shod!'
    end
    if matches[2] == 'clean' then
      if matches[3] == 'modlist' then
        ...
        local name = user_print_name(msg.from)
        savelog(matches[1], name.." ["..msg.from.id.."]User ["..user_id.." ] Mod list ra pak kard!")
      end
      ...
      if matches[3] == 'about' then
        ...
        save_data(_config.moderation.data, data)
        local name = user_print_name(msg.from)
        savelog(matches[1], name.." ["..msg.from.id.."]User ["..user_id.." ] About gorooh ra pak kard")
      end
    end
    if matches[2] == "setflood" then
      ...
      if tonumber(matches[3]) < 5 or tonumber(matches[3]) > 20 then
        return "User ["..user_id.." ] Khata!Lotfan adade morede nazaretono beyne adade [5ta20]entekhab konid!"
      end
      local flood_max = matches[3]
      data[tostring(matches[1])]['settings']['flood_msg_max'] = flood_max
      save_data(_config.moderation.data, data)
      local name = user_print_name(msg.from)
      savelog(matches[1], name.." ["..msg.from.id.."]User ["..user_id.." ] set flood be ["..matches[3].."]")
      return 'User ['..user_id..' ]floode gorooh set shavad be '..matches[3]
    end
    if matches[2] == 'lock' then
      ...
      if matches[3] == 'name' then
        local name = user_print_name(msg.from)
        savelog(matches[1], name.." ["..msg.from.id.."]User ["..user_id.." ] Esm ghofl shode ast! ")
        return lock_group_namemod(msg, data, target)
      end
      if matches[3] == 'member' then
        local name = user_print_name(msg.from)
        savelog(matches[1], name.." ["..msg.from.id.."]User ["..user_id.." ] Member ghofl shode ast! ")
    ...
    if matches[2] == 'unlock' then
      ...
      if matches[3] == 'name' then
        local name = user_print_name(msg.from)
        savelog(matches[1], name.." ["..msg.from.id.."]User ["..user_id.." ] Esme gorooh baz shode ast! ")
        return unlock_group_namemod(msg, data, target)
      end
      if matches[3] == 'member' then
        local name = user_print_name(msg.from)
        savelog(matches[1], name.." ["..msg.from.id.."]User ["..user_id.." ] Member baz shode ast! ")
    ...
    if matches[2] == 'get' then 
      if matches[3] == 'link' then
        ...
        if not group_link then 
          return "User ["..user_id.." ]baraye sakhte link /newlink ra type konid! first !"
        end
        local name = user_print_name(msg.from)
    ...
    if matches[1] == 'changeabout' and matches[2] and is_owner2(msg.from.id, matches[2]) then
      ...
      local name = user_print_name(msg.from)
      savelog(matches[2], name.." ["..msg.from.id.."]User ["..user_id.." ] etelaate gorooh ra taghir dad be ["..matches[3].."]")
    ...
    if matches[1] == 'changerules' and is_owner2(msg.from.id, matches[2]) then
      ...
      local target = matches[2]
      local name = user_print_name(msg.from)
      savelog(matches[2], name.." ["..msg.from.id.."]User ["..user_id.." ] Ghavanine goroh ro taghir dad be ["..matches[3].."]")
